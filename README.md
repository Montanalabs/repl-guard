...
Untrusted input is an expression; it can only ever become one of a fixed set of decisions over a
...
- **Untrusted input:** `fetch<web>` — an expression
- **Closed type:** `type Decision = EvalExpr(ReplOp) | RejectRepl` where `ReplOp` is closed
- **Sink / capability:** `grant evalRepl` → `privileged { evalRepl(d) }`
- **Consequence axes:** Trust

## Run the demo

```sh
examples/apps/repl-guard/demo.sh
```
...
- `repl-guard_safe.eide` — the correct design.
- `repl-guard_unsafe.eide` — the tempting-but-wrong version (the negative example a model must learn to reject).
